...
//   return (
//     <Paper elevation={3} style={{ position: 'fixed', bottom: '20px', right: '20px', width: '300px', height: '400px' }}>
//       <div style={{ maxHeight: '300px', overflowY: 'scroll', height: '80%' }}>
...
const ChatWindow = ({ onClose }) => {
  ...
  const handleSend = async () => {
    if (userInput.trim() !== '') { 
      try {
        const response = await axios.post('http://localhost:8000/chat_bot/ai', { text: userInput });
        const chatResponse = response.data.response;
        setChatMessages([...chatMessages, { text: userInput, type: 'user' }, { text: chatResponse, type: 'bot' }]);
        setUserInput('');
      } catch (error) {
